...
class TestOTPAuth:
    ...
    def setup(self):
        ...
        self.user = User.objects.create_user(
            username="testuser", email=self.email, password=self.password
        )
        # Clear cache
        cache.clear()
        # Clear outbox
    ...
    def test_verify_otp_success(self):
        ...
        )  # Depends on dj-rest-auth config
    ...
    def test_verify_otp_invalid(self):
        ...
        response = self.client.post(
            "/verify-otp/", {"email": self.email, "otp": "654321"}
        )
    ...
    def test_verify_otp_expired_or_missing(self):
        ...
        response = self.client.post(
            "/verify-otp/", {"email": self.email, "otp": "123456"}
        )
